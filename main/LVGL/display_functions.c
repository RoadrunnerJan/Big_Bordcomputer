...
void set_Displays() {
    ...
    lv_disp_load_scr(objects.gauge_oil_pressure);
...
static void lvgl_flush_cb(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_map) {
    esp_lcd_panel_handle_t panel_handler = (esp_lcd_panel_handle_t)disp_drv->user_data;
    // Dank full_refresh = 1 senden wir immer den kompletten Buffer
    esp_lcd_panel_draw_bitmap(panel_handler, area->x1, area->y1, area->x2 + 1, area->y2 + 1, color_map);
}
...
void display_init(void)
{
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)LCD_HOST, &io_config, &(io_handle[0])));
    ...
    ESP_ERROR_CHECK(esp_lcd_panel_reset(panel_handle[0]));
        ...
        // Attach the LCD to the SPI bus
        ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)LCD_HOST, &io_config_2, &io_handle[1]));
        ...
        // Attach the LCD to the SPI bus
        ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)LCD_HOST, &io_config_3, &io_handle[2]));
        ...
        // Attach the LCD to the SPI bus
        ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)LCD_HOST, &io_config_4, &io_handle[3]));
...
void buffer_init() {
    ...
    buf[0] = heap_caps_malloc(LCD_1_H_RES * LCD_1_V_RES * sizeof(lv_color_t), MALLOC_CAP_1);
    memset(buf[0], 0, LCD_1_H_RES * LCD_1_V_RES * sizeof(lv_color_t));
    lv_disp_draw_buf_init(&(draw_buf)[0], buf[0], NULL, LCD_1_H_RES * LCD_1_V_RES);

    #if NUMBER_OF_DISPLAYS > 1
        buf[1] = heap_caps_malloc(LCD_2_H_RES * LCD_2_V_RES * sizeof(lv_color_t), MALLOC_CAP_2);
        memset(buf[1], 0, LCD_2_H_RES * LCD_2_V_RES * sizeof(lv_color_t));
        lv_disp_draw_buf_init(&(draw_buf)[1], buf[1], NULL, LCD_2_H_RES * LCD_2_V_RES);
    #endif
    #if NUMBER_OF_DISPLAYS > 2
        buf[2] = heap_caps_malloc(LCD_3_H_RES * LCD_3_V_RES * sizeof(lv_color_t), MALLOC_CAP_3);
        memset(buf[2], 0, LCD_3_H_RES * LCD_3_V_RES * sizeof(lv_color_t));
        lv_disp_draw_buf_init(&(draw_buf)[2], buf[2], NULL, LCD_3_H_RES * LCD_3_V_RES);
    #endif
    #if NUMBER_OF_DISPLAYS > 3
        buf[3] = heap_caps_malloc(LCD_4_H_RES * LCD_4_V_RES * sizeof(lv_color_t), MALLOC_CAP_4);
        memset(buf[3], 0, LCD_4_H_RES * LCD_4_V_RES * sizeof(lv_color_t));
        lv_disp_draw_buf_init(&(draw_buf)[3], buf[3], NULL, LCD_4_H_RES * LCD_4_V_RES);
    #endif
